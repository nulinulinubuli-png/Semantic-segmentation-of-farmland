...
class VOCSegmentation(Dataset):
    ...
    def __init__(self,
        ...
        for splt in self.split:
            ...
            for ii, line in enumerate(lines):
                ...
                _g_image = os.path.join(self._image_dir, line + "_" + self.args.suffix[1] + ".tif")
                _m_image = os.path.join(self._image_glcm, line + "_" + self.args.suffix[0] + "_homogeneity.png")
                print(_m_image)
                _l_image = os.path.join(self._image_glcm, line + "_" + self.args.suffix[0] + "_entropy.png")
                ...
                self.mimages.append(_m_image)
                self.limages.append(_l_image)
    ...
    def _make_img_gt_point_pair(self, index):
        ...
        _m_img1 = Image.open(self.mimages[index]).convert('RGB')
        _l_img = Image.open(self.limages[index]).convert('RGB')

        _img = np.concatenate((_img, _m_img,_m_img1,_l_img), axis=-1)
    ...
    def transform_tr(self, sample):
        composed_transforms = transforms.Compose([
            ...
            tr.ImsRandomScaleCropv4(base_size=self.args.base_size, crop_size=self.args.crop_size),
            tr.ImsRandomGaussianBlurv4(),
            tr.ImsNormalizev4(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)),
            tr.ToTensor()])
    ...
    def transform_val(self, sample):
        ...
        composed_transforms = transforms.Compose([
            tr.ImsFixScaleCropv4(crop_size=self.args.crop_size),
            tr.ImsNormalizev4(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)),
            tr.ToTensor()])
